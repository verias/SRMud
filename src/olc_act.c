...
/* ROM specific bits: */
	...
	{   "clanedit",	clan_flags,		 "Clan flags."			},
    {	NULL,		NULL,		 NULL				 }
};
...
REDIT( redit_delete )
{
	...
	if( ch->in_room->vnum == index )
	{
		...
		char_from_room( ch );
		char_to_room( ch, get_room_index( 2 ) ); /* limbo */
...
const struct wear_type wear_table[] =
{
	...
	{	WEAR_LODGE_ARM, ITEM_LODGED_BODY	},
	{	WEAR_LODGE_LEG, ITEM_LODGED_BODY	},
	{	WEAR_LODGE_RIB, ITEM_LODGED_BODY	},
    {	NO_FLAG,	NO_FLAG			}
};
...
/* It somehow fixed a bug in showing scroll/pill/potions too ?! */
	...
	case ITEM_ORE:
		sprintf(buf,
			...
			"[v3] Workability:  [%d]\n\r"
			"[v4] Color:        [%s]\n\r",
			smith_ores_table[obj->value[0]].name,
			obj->value[1],
			obj->value[2],
			obj->value[3],
			smith_ores_table[obj->value[4]].color);
		send_to_char(buf,ch);
		break;
...
case ITEM_ARROW:
		...
		case ITEM_BLOOD:
			...
			{
			default:
				send_to_one(ch,"Age of bloodtrail.");
				return FALSE;
			case 0:
				send_to_one(ch,"AGE OF BLOOD SET.");
				pObj->value[0] = atoi( argument);
				break;
			}
			break;
...
/* OLC		    do_help( ch, "liquids" );    */
		...
		case ITEM_TOKEN:
			...
			{
			default:
				send_to_one(ch,"Token types, values for winning and if it's a machine or not.");
				return FALSE;
			case 0:
				send_to_char("TOKEN TYPE SET.\n\r\n\r",ch);
				pObj->value[0] = (token_lookup( argument ) != -1 ?
					token_lookup(argument) : 0 );
				break;
			case 1:
				send_to_char("WIN CHANCE SET.\n\r",ch);
				pObj->value[1]	=	atoi(argument);
				break;
			case 2:
				send_to_char("MAXIMUM WINNINGS SET.\n\r",ch);
				pObj->value[2]	= atoi(argument);
				break;
			case 3:
				send_to_char("MINIMUM WINNINGS SET.\n\r",ch);
				pObj->value[3]	= atoi(argument);
				break;
			case 4:
				send_to_char("MACHINe SET.\n\r",ch);
				pObj->value[4]	= atoi(argument);
				break;
			}
			break;
			...
			case ITEM_ORE:
				...
				{
				default:
					send_to_one(ch,"Name, cost, armor mod, workability and color.");
					return FALSE;
				case 0:
					send_to_one(ch, "NAME SET");
					pObj->value[0] = ( ore_lookup(argument) != -1 ? 
						ore_lookup(argument) : 0 );
					break;
				case 1:
					send_to_one(ch,"COST SET");
					pObj->value[1] = atoi(argument);
					break;
				case 2:
					send_to_one(ch,"ARMOR MOD SET");
					pObj->value[2] = atoi(argument);
					break;
				case 3:
					send_to_one(ch,"WORKABILITY SET");
					pObj->value[3] = atoi(argument);
					break;
				case 4:
					send_to_one(ch,"COLOR SET");
					pObj->value[4] = (ore_color_lookup(argument) != -1 ?
						ore_color_lookup(argument) : 0);
						break;
				}
				break;
...
CLEDIT( cledit_flags )
{
  ...
  if ( argument[0] != '\0' )
  {
    ...
    {
      pClan->flags ^= value;
      send_to_char( "Clan flag(s) toggled.\n\r", ch);
      return TRUE;
    }
  }
...
CLEDIT( cledit_create )
{
  ...
  if (i <= MAX_CLAN) /* open slot */
  {
    ...
    clan_table[i].name = str_dup("New Clan");
    clan_table[i].who_name = str_dup("New Clan");
  ...
  send_to_char("No room to create a new clan. Increase MAX_CLAN\n\r", ch);  
  return FALSE;
} 
...
CLEDIT( cledit_list )
{
  ...
  sprintf(buf, "Num  Clan  Name  Flags\n\r-----------------------------------------------\n\r");
  add_buf(buffer, buf);
  ...
  for (i=1; i <= MAX_CLAN; i++)
  {
    if (clan_table[i].name != NULL && clan_table[i].name[0] != '\0')
    {
      sprintf(buf,"[%2d]  %s\t Flags:\t[%s]\n\r", 
                  i, 
                  clan_table[i].name,
                  flag_string( clan_flags, clan_table[i].flags ) );
      add_buf(buffer, buf);
    }
  }
...
CLEDIT( cledit_show )
{
  ...
  sprintf(buf, "Name     : %s %s %s\n\rWho Name : %-10s\n\r",
               pClan->name, 
               IS_SET(pClan->flags, CLAN_CHANGED) ? "`c[`B*`c]`w" : "",
               IS_SET(pClan->flags, CLAN_DELETED) ? "`RMarked for Deletion!`w" : "",
               pClan->who_name);
  add_buf(buffer, buf);
