...
int token_lookup(const char *name)
{
    int sn;

    for (sn = 0; sn < MAX_TOKEN; sn++)
    {
	if (LOWER(name[0]) == LOWER(token_table[sn].name[0])
	&&  !str_prefix(name,token_table[sn].name))
	    return sn;
    }
...
int ore_color_lookup(const char *name)
{
	int ore;

	for( ore = 0; smith_ores_table[ore].color != NULL; ore++)
	{
		if(LOWER(name[0]) == LOWER(smith_ores_table[ore].color[0])
			&& !str_prefix(name,smith_ores_table[ore].color))
			return ore;
