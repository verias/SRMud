...
//	one_argument(argument,arg);
	...
	{
	//			argument = one_argument(argument,arg2);   
		iClass = class_lookup(arg2);
		if (iClass == -1)
		{
			strcpy( buf2, "Possible Classes [");
			for ( iClass = 0; iClass < MAX_CLASS; iClass++ )
			{
			    if ( iClass > 0 )
				strcat( buf2, " " );
			    strcat( buf2, class_table[iClass].name );
			}
			strcat( buf2, "]: \n\r" );	
...
bool parse_gen_groups(CHAR_DATA *ch,char *argument)
{
	...
	if (sn != -1)
	{
	    ...
	    sprintf(buf, "%s skill added from parse_gen_groups",skill_table[sn].name);
	    send_to_char(buf,ch);
