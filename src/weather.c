...
{
        send_to_char("{g[{YWeather{g] {CRain is pouring down hard.",ch); 
}

else
  if ( number_range( 0, 100 ) <= 60  )
{
        send_to_char("{g[{YWeather{g] {CIt is raining in thick sheets.",ch); 
}
else
  if ( number_range( 0, 100 ) <= 75  )
{
        send_to_char("{g[{YWeather{g] {CPelts of rain can be felt through your clothing.",ch); 
}
else   if ( number_range( 0, 100 ) <= 85  )
{
        send_to_char("{g[{YWeather{g] {CA heavy downpour falls all about.",ch); 
}
else
  if ( number_range( 0, 100 ) <= 100  )
{
        send_to_char("{g[{YWeather{g] {CIt's raining enough to flood a small city.",ch); 
}
    ...
    send_to_char( buf, ch );
        ...
        {
        send_to_char("{g[{YWeather{g] {CMarble like substances are falling from the sky.{x\n\r",ch);
        }
...
void show_weather(CHAR_DATA *ch)
{
	...
	if(ch->in_room->room_flags != ROOM_INDOORS)
	{
/*for weather stuff -- tjw*/
  ...
  if ( number_range( 0, 100 ) <= 15  )
{
        send_to_char("{CThere is a cold rain trickling down.{x\n\r",ch);
}
else
  if ( number_range( 0, 100 ) <= 30  )
{
        send_to_char("{CThere is a warm rain trickling down.{x\n\r",ch);
}
else   if ( number_range( 0, 100 ) <= 45  )
{
        send_to_char("{CRain is pouring down hard.{x\n\r",ch); 
}
else
  if ( number_range( 0, 100 ) <= 60  )
{
        send_to_char("{CIt is raining in thick sheets.{x\n\r",ch); 
}
else
  if ( number_range( 0, 100 ) <= 75  )
{
        send_to_char("{CPelts of rain can be felt through your clothing.{x\n\r",ch); 
}
else   if ( number_range( 0, 100 ) <= 85  )
{
        send_to_char("{CA heavy downpour falls all about.{x\n\r",ch); 
}
else
  if ( number_range( 0, 100 ) <= 100  )
{
        send_to_char("{CIt's raining enough to flood a small city.{x\n\r",ch); 
}
//	    	    send_to_char( "{x\n\r",ch);
    ...
    send_to_char( buf, ch );
        ...
        {
        send_to_char("{CThe sky is beautiful, not a cloud around.{x\n\r",ch);
        }
                 else if (weather_info.sky == SKY_THUNDERSTORM)
        {
        send_to_char("{CThe skies thunder as a storm approaches.{x\n\r",ch);
        }
                 else if (weather_info.sky == SKY_ICESTORM)
        {
        send_to_char("{CSheets of ice appear to be falling from the sky.{x\n\r",ch);
        }
                 else if (weather_info.sky == SKY_HAILSTORM)
        {
        send_to_char("{CMarble like substances are falling from the sky.{x\n\r",ch);
        }
                 else if (weather_info.sky == SKY_SNOWING)
        {
        send_to_char("{CA light snow is falling.{x\n\r",ch);
        }
                 else if (weather_info.sky == SKY_BLIZZARD)
        {
        send_to_char("{CThere is a blizzard about.{x\n\r",ch);
       }
                 else if (weather_info.sky == SKY_FOGGY)
        {
        send_to_char("{CA misty haze covers the horizon.{x\n\r",ch);
        }

        else if (weather_info.sky == SKY_LIGHTNING)
        {
        send_to_char("{CA {y Lightning{w storm is approaching {x\n\r",ch);
     }
        else 
        {
        send_to_char("{CBUG!!!!!!!!!!!!!!! A descriptor does not exist for this condition  {x\n\r",ch);
        }
	}
}
