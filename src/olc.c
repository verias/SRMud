 ...
 * Local functions.
 */
extern ORGANIZATION *org_lookup args( (const char * ));
...
bool show_commands( CHAR_DATA *ch, char *argument )
{
	...
	case ED_HELP:
	    ...
	    break;
	case ED_ORG:
		show_olc_cmds( ch, orgedit_table );
		break;
	case ED_CLAN:
...
const struct olc_cmd_type orgedit_table[] =
{
	{ "show",			orgedit_show	},
	{ "name",			orgedit_name	},
	{ "patron",			orgedit_patron	},
	{ "rank",			orgedit_rank	},
//	{ "skill",			orgedit_skill	},
	{ "new",			orgedit_new		},
	{ "list",			orgedit_list	},
	{ "delete",			orgedit_delete	},
	{ "command",		show_commands	},
	{ "message",		orgedit_message	},
	{ "type",			orgedit_type	},
	{ "file",			orgedit_file	},
	{ NULL,					0			}
};
...
void cledit( CHAR_DATA *ch, char *argument )
{
  ...
  for ( cmd = 0; cledit_table[cmd].name != NULL; cmd++ )
  {
    if ( !str_prefix( command, cledit_table[cmd].name ) )
    {
      if ( (*cledit_table[cmd].olc_fun) ( ch, argument ) )
      {
...
// Organization Interpreter called by do_orgedit
void orgedit (CHAR_DATA *ch, char *argument )
{
	char organization[MIL];
	char arg[MSL];
	int org;
	ORGANIZATION *pOrg;
	...
	smash_tilde(argument);
	strcpy(arg,argument);
	argument = one_argument(argument,organization);

	EDIT_ORG(ch, pOrg);

	if(!str_cmp(organization, "done"))
	{
		edit_done(ch);
		return;
	}
	...
	if(organization[0] == '\0')
	{
		orgedit_show(ch,argument);
		return;
	}

	for (org = 0; orgedit_table[org].name != NULL; org++)
	{
		if(!str_prefix( organization, orgedit_table[org].name))
		{
			orgedit_table[org].olc_fun(ch,argument);
			return;
		}
...
void do_orgedit( CHAR_DATA *ch, char *argument)
{
	...
	if(arg[0] == '\0')
	{
		send_to_one(ch,"Syntax: OrgEdit <organization>");
		return;
	}
	if (!str_cmp(arg,"new"))
	{
		orgedit_new(ch,argument);
		return;
	}

	if(!str_cmp(arg,"delete"))
	{
		orgedit_delete(ch,argument);
		return;
	}

	if(!str_cmp(arg,"list"))
	{
		orgedit_list(ch,argument);
		return;
	}

	if((org = org_lookup(arg)) == NULL)
	{
		send_to_one(ch,"There is no organization by that name.");
		return;
	}

	ch->desc->pEdit = (void *) org;
	ch->desc->editor = ED_ORG;
	send_to_one(ch,"Entering the Organization editor for %s.",org->name);
	return;
}
...
void do_cledit( CHAR_DATA *ch, char *argument )
{
	...
	{
	  send_to_char( "GEdit : Insuffecient security to create new clans.\n\r", ch );
  	  return;
	}
    ...
    if (ch->level < MAX_LEVEL -2 )
    {
	send_to_char("Insuffecient security to edit clans.\n\r",ch);
	return;
    }
...
void display_resets( CHAR_DATA *ch )
{
  ...
  "\n\r", ch );
	...
	case 'M':
	    ...
	    /*
	     * Check for pet shop and mount shop.
	     * -------------------
	     */
	    {
