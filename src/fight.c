...
if(arrow->item_type== ITEM_ARROW )
   {
	...
	case 6 :
			...
			arrow->wear_flags = ITEM_LODGED_BODY;
			arrow->wear_loc = WEAR_LODGE_ARM;
			wear_obj(victim, arrow,TRUE);
			arrow->wear_flags = ITEM_HOLD;
			arrow->wear_flags = arrow->wear_flags + ITEM_TAKE;
...
bool check_static_shield( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    send_to_one(victim,"Your static shield catches %s!",ch->name);
    send_to_one(ch,"%s's static shield catches you!", victim->name);
...
void do_stake( CHAR_DATA *ch, char *argument )
{
  ...
  one_argument(argument,arg);
	...
	{
		send_to_one(ch,"You are dead and cannot do that.");
		return;
	}
  ...
  if ( (chance = get_skill(ch,gsn_stake)) &&
	...
	{
		send_to_one(ch,"They are already dead.");
		return;
	}
    ...
    */
	...
	damage(ch,victim,((ch->level*(dice((int)ch->level/4,6))) + ch->level),gsn_stake,
	    DAM_PIERCE,TRUE);
...
int xp_compute( CHAR_DATA *gch, CHAR_DATA *victim, int total_levels )
{
	...
	case -7 :	base_exp =   3;		break;
	case -6 : 	base_exp =   5;		break;
	case -5 :	base_exp =   9;		break;
	case -4 :	base_exp =  11;		break;
	case -3 :	base_exp =  22;		break;
	case -2 :	base_exp =  27;		break;
	case -1 :	base_exp =  30;		break;
	case  0 :	base_exp =  39;		break;
	case  1 :	base_exp =  50;		break;
	case  2 :	base_exp =  67;		break;
	case  3 :	base_exp =  75;		break;
	case  4 :	base_exp =  82;		break;
    } 
    
    if (level_range > 4)
	base_exp = 100 + 20 * (level_range - 4);
      ...
      if(IS_NPC(victim) )/* at max a mob with all worth 5 level above his own*/
       {
         ...
         if( IS_SET(victim->off_flags,OFF_BACKSTAB) ) 
            base_exp = (base_exp * 120) / 100;
         if( IS_SET(victim->off_flags,OFF_CIRCLE) ) 
            base_exp = (base_exp * 120) / 100;
...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim,int dam,int dt,bool immune )
{
	...
	else
	{
	    ...
	    else
	    {
	    	...
	    	sprintf( buf2, "{2Your %s %s $N%c{x {D[{G%d{D]{x",  attack, vp, punct, dam );
	    	sprintf( buf3, "{4$n's %s %s you%c{x {D[{R%d{D]{x", attack, vp, punct, dam );
	    }
	}
    }
...
void do_slay( CHAR_DATA *ch, char *argument )
{
    ...
    act( "{5You slay $M in cold blood!{x",  ch, NULL, victim, TO_CHAR    );
    act( "{5$n slays you in cold blood!{x", ch, NULL, victim, TO_VICT    );
    act( "{5$n slays $N in cold blood!{x",  ch, NULL, victim, TO_NOTVICT );
    raw_kill( victim );
    return;
}
