    ...
    switch ( pObjIndex->item_type )
    {
        ...
        case ITEM_ORE:
			fprintf( fp, "%s %d %d %d %s\n",
					smith_ores_table[pObjIndex->value[0]].name,
					pObjIndex->value[1],
					pObjIndex->value[2],
					pObjIndex->value[3],
					smith_ores_table[pObjIndex->value[4]].color);
			break;
...
void save_clans(CHAR_DATA *ch, char *argument)
{
  ...
  {
    if ( !IS_SET(clan_table[i].flags, CLAN_DELETED) && clan_lookup(clan_table[i].name) != 0)
    {
      fprintf(fp, "\nGuild\t%s~\n", clan_table[i].name);
  ...
  fprintf(fp, "\nEnd\n");
  fclose(fp);
  send_to_char("Clan data file (clan.dat) written.\n\r", ch);
  return;        
}
...
void save_area( AREA_DATA *pArea )
{
    FILE *fp;
	char strsave[MIL];
	char strbackup[MIL+4];

    fclose( fpReserve );
	sprintf(strsave, "%s",pArea->file_name);
...
//	_unlink( strbackup );
	...
	rename(strbackup,pArea->file_name);
