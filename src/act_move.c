...
char * const rev_name [] =
{
	"south", "west", "north", "east", "down", "up", "southwest",
...
//	    act( "$n leaves $T.", ch, NULL, dir_name[door], TO_ROOM );
	...
	was_room = ch->in_room;
			...
			percent = (ch->hit / ch->max_hit)*100;
		if(ch->hit < (ch->max_hit / 4)*3)
		{
			//OBJ_DATA *obj, *obj_next;
			OBJ_DATA *bObj, *bObj_next;
			char buf[MIL];

//			for(obj = was_room->contents; obj; obj = obj_next)
//			{
//			obj_next = obj->next_content;
			for(bObj = was_room->contents; bObj; bObj = bObj_next)
			{
				bObj_next = bObj->next_content;
			if(bObj->item_type != ITEM_BLOOD )	
				continue;

			extract_obj(bObj);
			}
			bObj = create_object(get_obj_index(OBJ_VNUM_BLOOD),0);
			bObj->timer = number_range(1,5);
			sprintf(buf, bObj->description, dir_name[door]);
			free_string(bObj->description);
			bObj->description = str_dup(buf);
			bObj->owner = str_dup(ch->name);
						
			obj_to_room(bObj, was_room);
			}
	//	}
...
//	act( "$n has arrived.", ch, NULL, NULL, TO_ROOM );
	...
	{
		    if (!IS_NPC(ch) && ch->pcdata->condition[COND_DRUNK] > 10)
        act("$n stumbles in drunkenly.", ch,NULL,NULL,TO_ROOM);
			else
	    act( "$n has arrived.", ch, NULL, NULL, TO_ROOM );
	}
	else
...
if ( MOUNTED(ch) )
{
	send_to_one(ch,"You can't jog while mounted!");
	return;
}
*/
buf[0] = '\0';
