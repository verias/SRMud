...
//    control = init_socket( port );
	...
	boot_db( );
    sprintf( log_buf, "Shattered Reality is ready to rock on port %d.", port );
    log_string( log_buf );
...
void init_descriptor( int control )
{
	...
	log_string("initialize descriptor");
    size = sizeof(sock);
...
bool read_from_descriptor( DESCRIPTOR_DATA *d )
{
	...
	else if ( WOULD_HAVE_BLOCKED || errno == EAGAIN )
	    break;
	else
	{
     ...
     if (*d->run_head != '\0')
     {
       ...
       else
       {
         ...
         switch( *d->run_head )
         {
		   ...
		   case 'ne' : sprintf( buf+strlen(buf), "northeast");break;
		   case 'nw' : sprintf( buf+strlen(buf), "northwest");break;
		   case 'se' : sprintf( buf+strlen(buf), "southeast");break;
		   case 'sw' : sprintf( buf+strlen(buf), "southwest");break;
           default: return;
         }
...
void nanny( DESCRIPTOR_DATA *d, char *argument )
{
    ...
    int race,i;
    bool fOld;
	int etrait, htrait, height, weight;
	int sex, day;
...
/* RT code for breaking link */
	...
	for ( race = 1; race_table[race].name != NULL; race++ )
	{
	    ...
	    if ( !race_table[race].remort_race ||
	       (race_table[race].remort_race && ch->pcdata->incarnations > 1))
	    {
	...
	ch->size = pc_race_table[race].size;
       ...
       send_to_desc("\n\r",d);
       send_to_desc("[ red blue green black brown purple grey white hazel ]\n\r",d);
        write_to_buffer(d,"\n\r",0);
...
case CON_GET_ETRAIT:
      ...
      send_to_desc("[ red blue green black brown purple grey white hazel ]\n\r",d);
       ...
       send_to_desc("\n\r",d);
       send_to_desc("[ red blue green black brown purple grey white orange silver yellow ]\n\r",d);
        write_to_buffer(d,"\n\r",0);
...
case CON_GET_HTRAIT:
      ...
      send_to_desc("[ red blue green black brown purple grey white orange silver yellow ]\n\r",d);
            write_to_buffer(d,
                "What is your characters hair color?",0);
            break;
         }
        ...
        write_to_buffer(d,"\n\r",0);
        write_to_buffer(d,"What is your characters height?\n\r",0);
		write_to_buffer(d,"[ ",0);
		for( height = 0; height_table[height].name != NULL; height++)
		{
			if(height_table[height].can_be_size[ch->race])
			{
			write_to_buffer(d,height_table[height].name,0);
			write_to_buffer(d," ",1);
			}
		}
	write_to_buffer(d,"] :",0);
...
//	one_argument(argument,arg);
	...
	height = height_lookup(argument);
	if( height == -1 || !height_table[height].can_be_size[ch->race])
	{
		write_to_buffer(d,"That is not a valid height.\n\r",0);
		write_to_buffer(d,"What is your character's height?\n\r",0);
		write_to_buffer(d,"[ ",0);
		for( height = 0; height_table[height].name != NULL; height++)
		{
			if(height_table[height].can_be_size[ch->race])
	...
	ch->pcdata->height = height;
		write_to_buffer( d, "What is your sex? ", 0 );
		write_to_buffer( d, "[ ",0);
		for(sex = 0; sex_table[sex].name != NULL; sex++)
		{
			write_to_buffer(d,sex_table[sex].name,0);
			write_to_buffer(d," ",1);
		}
		write_to_buffer(d,"]\n\r",0);
	...
	sex = sex_lookup(argument);

	if( sex == -1 )
	{
		write_to_buffer(d,"That is not a valid sex.\n\r",0);
		write_to_buffer(d,"What is your sex?\n\r",0);
				write_to_buffer( d, "[ ",0);
		for(sex = 0; sex_table[sex].name != NULL; sex++)
		{
			write_to_buffer(d,sex_table[sex].name,0);
		...
		write_to_buffer(d,"What is your character's build?\n\r",0);
	write_to_buffer(d,"[ ",0);
		for( weight = 0; weight_table[weight].name != NULL; weight++)
		{
			if((weight_table[weight].can_be_male && ch->sex == 0)
			|| (weight_table[weight].can_be_female && ch->sex == 1)
			|| (weight_table[weight].can_be_either && ch->sex == 2)
			|| (weight_table[weight].can_be_none && ch->sex == 3))
			{
		write_to_buffer(d,weight_table[weight].name,0);
		write_to_buffer(d," ",1);
			}
		}
	write_to_buffer(d,"] :",0);
	...
	case CON_GET_WEIGHT:
//	one_argument(argument,arg);
	weight = weight_lookup(argument);

	if(weight == -1 ||
		(!weight_table[weight].can_be_male && ch->sex == 0)
			|| (!weight_table[weight].can_be_female && ch->sex == 1)
			|| (!weight_table[weight].can_be_either && ch->sex == 2)
			|| (!weight_table[weight].can_be_none && ch->sex == 3))
	{
		write_to_buffer(d,"That is not a valid build.\n\r",0);
		write_to_buffer(d,"What is your character's build?\n\r",0);
		write_to_buffer(d, "[ ",0);
		for( weight = 0; weight_table[weight].name != NULL; weight++)
		{
		...
		sprintf(buf2,"%s",ch->sex == 0 ? "G" :
		ch->sex == 1 ? "M" :
		ch->sex == 3 ? "D" : "W");
		sprintf(buf,"A %s, %s {%s%s{x with %s eyes and %s hair",height_table[ch->pcdata->height].name,
		weight_table[ch->pcdata->weight].name, buf2,race_table[ch->race].name, etrait_table[ch->etrait].ecolor,
		htrait_table[ch->htrait].hcolor);
...
//    send_to_desc("Do you want ANSI? (Y/n) ",d);
	...
	write_to_buffer( d, 
    "\n\rWelcome to Shattered Reality.  Please do not feed the mobiles.\n\r",
	    0 );
	ch->next	= char_list;
	char_list	= ch;
	d->connected	= CON_PLAYING;
	...
	if ( ch->level == 0 )
	{
		...
		ch->class = class_lookup("default");
		ch->pcdata->birthday = day;
		ch->pcdata->birthmonth = time_info.month;
		ch->pcdata->birthyear  = time_info.year - 17;
	    ch->level	= 1;
...
{
    static char * const he_she  [] = { "he",  "she", "it" };
    static char * const him_her [] = { "him", "her", "it" };
    static char * const his_her [] = { "his", "her", "its" };
...
int colour( char type, CHAR_DATA *ch, char *string )
{
	...
	case '9':
		...
		sprintf( code, "\033[%d;3%dm", col->gossip_text[0], col->gossip_text[1] );
	    break;
	case 'Z':
	    if( col->wiznet[2] )
		sprintf( code, "\033[%d;3%dm%c",
	...
	case 'O':
		...
		sprintf( code, "\033[%d;3%dm", col->room_things[0], col->room_things[1] );
	    break;
	case 'i':
	...
	case '5':
		...
		sprintf( code, "\033[%d;3%dm", col->fight_skill[0], col->fight_skill[1] );
	    break;
	case '1':
		...
		sprintf( code, "\033[%d;3%dm", col->fight_death[0], col->fight_death[1] );
	    break;
	case '6':
	    if( col->say[2] )
	...
	case 'A':
		...
		sprintf( code, "\033[%d;3%dm", col->auction_text[0], col->auction_text[1] );
	    break;
	case 'q':
	    if( col->question[2] )
		sprintf( code, "\033[%d;3%dm%c",
	...
	case 'F':
		...
		sprintf( code, "\033[%d;3%dm", col->answer_text[0], col->answer_text[1] );
	    break;
	case 'e':
	    if( col->music[2] )
		sprintf( code, "\033[%d;3%dm%c",
