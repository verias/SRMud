...
void do_cast( CHAR_DATA *ch, char *argument )
{
	...
	if(IS_SET(victim->act,PLR_DEAD) && sn != find_spell(ch,"resurrect") )
	{
...
//	OBJ_DATA *money;
	...
	{
		send_to_one(ch,"They aren't dead.");
		return;
	}
	...
	for(corpse = ch->in_room->contents; corpse; corpse = corpse->next_content)
	{
		...
		{
			found = TRUE;
			if(victim->in_room != ch->in_room || str_cmp(corpse->owner,victim->name))
			{
				send_to_one(ch,"The spirit and the corpse must be in the same room.");
				return;
			}

		act("$n utters an ancient incantation and $N breathes once more.",ch,NULL,victim,TO_NOTVICT);
		act("You utter an ancient incantation and $N breathes once more.",ch,NULL,victim,TO_CHAR);
		act("$n utters a brief incantation and you feel your lungs fill with air once more.",ch,NULL,victim,TO_VICT);
		/*		for( obj = corpse->contains; obj != NULL; obj = obj_next)
		{
         ...
         {
			do_function(victim, &do_get, "all corpse");
	...
	if(!found)
	{
		send_to_one(ch,"You corpse and target must be in the same room.");
		return;
	}
}
...
void spell_identify( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
	...
	switch (obj->value[0])
	{
		...
		case(WEAPON_BOW)	: send_to_one(ch,"bow.");			break;
