...
int weight_lookup (const char *cweight)
 {
    int weight;

    for ( weight = 0; weight_table[weight].name != NULL; weight++)
    {
         if (LOWER(cweight[0]) == LOWER(weight_table[weight].name[0])
         &&  !str_prefix( cweight,weight_table[weight].name))
             return weight;
    }
...
int height_lookup (const char *cheight)
 {
    int height;

    for ( height = 0; height_table[height].name != NULL; height++)
    {
         if (LOWER(cheight[0]) == LOWER(height_table[height].name[0])
         &&  !str_prefix( cheight,height_table[height].name))
             return height;
    }
...
{
    return time_info.year - ch->pcdata->birthyear;
}
...
void equip_char( CHAR_DATA *ch, OBJ_DATA *obj, int iWear )
{
    ...
    if ( get_eq_char( ch, iWear ) != NULL && !IS_SET(obj->wear_flags,ITEM_LODGED_BODY))
    {
...
void extract_char( CHAR_DATA *ch, bool fPull )
{
    ...
    */
    ...
    if ( !fPull )
    {
        char_to_room(ch,room);//,get_room_index(get_clan_table(ch->clan)->hall));
	return;
    }
	...
	if ( ch->mount && ch->mount->mount == ch )
	{
	    ...
	    if (ch->mount->riding)
	    {
		act("You fall off of $N.", ch, NULL, ch->mount, TO_CHAR);
		act("$n falls off of $N.", ch, NULL, ch->mount, TO_ROOM);
...
bool can_see_room( CHAR_DATA *ch, ROOM_INDEX_DATA *pRoomIndex )
{
    ...
    if (!IS_IMMORTAL(ch) && pRoomIndex->clan && (ch->clan != pRoomIndex->clan || ch->city != pRoomIndex->clan || ch->guild != pRoomIndex->clan))
	return FALSE;
...
char *wear_bit_name(int wear_flags)
{
	...
	if (wear_flags & ITEM_LODGED_BODY	) strcat(buf, " lodged");
