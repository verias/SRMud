...
SEDIT( sedit_show )
{
	...
	sprintf (buf, "Social: %s\n\r"
	              ...
	              "%s\n\r\n\r"
	              "(onoarg) Without argument, other see :\n\r"
	              "%s\n\r\n\r"
	              "(cfound) With target, player sees :\n\r"
	              "%s\n\r\n\r"
	              "(ofound) With target, others see :\n\r"
	              "%s\n\r\n\r"
	              "(vfound) With target, victim sees :\n\r"
	              "%s\n\r\n\r"
	              "(cself) target is the same player :\n\r"
	              "%s\n\r\n\r"
	              "(oself) target is the same player, others see :\n\r"
	              "%s\n\r",
...
SEDIT( sedit_new )
{
	...
	if ( IS_NULLSTR(argument) )
	{
		send_to_char( "Syntax : new [name-of-new-social]\n\r", ch );
		return FALSE;
	}
	...
	{
		send_to_char ("Failure in realloc. Prepare for the impact.\n\r",ch);
		return FALSE;
	}
...
SEDIT( sedit_delete )
{
	...
	if ( IS_NULLSTR(argument) )
	{
		send_to_char( "Sintaxis : delete [name-of-social-to-erase]\n\r", ch );
		return FALSE;
	}
	...
	send_to_char ("That social has been erased.\n\r",ch);
	return TRUE;
}
#endif
