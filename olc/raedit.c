...
void do_raedit(CHAR_DATA *ch, char *argument)
{
    ...
    {
    	send_to_char( "Syntax : RAEdit [raza]\n\r", ch );
    	return;
    }
...
RAEDIT( raedit_new )
{
	...
	{
		send_to_char ("Failure in realloc. Brace for impact.\n\r",ch);
		return FALSE;
	}
...
RAEDIT( raedit_cmult )
{
	...
	{
		send_to_char( "RAEdit : Non-existant class.\n\r", ch );
		return FALSE;
	}
	...
	if ( mult < 1 || mult > 200 )
	{
		send_to_char( "RAEdit : Invalid multiplier.\n\r", ch );
		return FALSE;
	}
...
RAEDIT( raedit_stats )
{
	...
	if ( IS_NULLSTR(argument) )
	{
		send_to_char( "Syntax : stats [stat] [valor]\n\r", ch );
		return FALSE;
	}
