...
void load_struct( FILE *fp, void * tipobase, const struct savetable_type * tabla, void * puntero )
{
	...
	while ( str_cmp((word = fread_word(fp)), "#END") )
	{
				...
				switch(temp->tipo_campo)
				{
					...
					*pint = fread_number(fp);
					...
					if ((*funcion) (pint, cadena) == FALSE)
						bugf( "load_struct : field %s invalid, string %s",
							temp->campo, cadena );
					free_string(cadena);
					found = TRUE, cnt++;
					break;
					...
					*pbool = str_cmp( cadena, "false" ) ? TRUE : FALSE;
					...
					while( str_cmp((cadena = fread_word(fp)), "@") )
					{
						if ( i == (int) temp->argumento )
							bugf( "load_struct : campo_shint_array %s with an excess of elements",
								temp->campo );
						else
					...
					{
						if ( i == (int) temp->argumento )
							bugf( "load_struct : campo_string_array %s with an excess of elements",
								temp->campo);
						else
							pcadena[i++] = cadena;
					}
					...
					while( str_cmp((cadena = fread_word(fp)), "@") )
					{
						...
						||   (temp->argumento == NULL
						  ...
						  && i == *( (int *) temp->argumento2)) )
							bugf( "load_struct : campo_bool_array %s with an excess of elements",
								temp->campo );
						else
		...
		if (found == FALSE)
		{
			bugf( "load_struct : key %s could not be found", word );
			fread_to_eol(fp);
		}
		else
			found = FALSE;
...
void save_struct( FILE *fp, void * tipobase, const struct savetable_type * tabla, void * puntero )
{
	...
	for ( temp = tabla; !IS_NULLSTR(temp->campo); temp++ )
	{
		...
		{
			default:
			bugf( "save_struct : tipo_campo %d invalid, string %s",
...
static	struct skill_type skzero;
	...
	for ( ; ; )
	{
		...
		{
			bugf( "Cargar_skills : key nonexistent (%s)", word );
			exit(1);
		}

		if ( i >= MAX_SKILL )
		{
			bug( "Cargar_skills : number of skills greater then MAX_SKILL", 0 );
			exit(1);
		}
