...
/*            **ch->desc->pString = '\0'; */
	...
	if ( !str_cmp( arg1, ".li" ) )
	{
		...
		{
			write_to_buffer( ch->desc, "Line too long.\n\r", 0 );
			return;
		}
        ...
        if ( !str_cmp( arg1, ".h" ) )
        {
            write_to_buffer( ch->desc,  "Sedit help (commands on blank line):   \n\r"
					".r 'old' 'new'   - replace a substring \n\r"
	    				...
	    				".ld <num>        - erase line <num>\n\r"
	    				".li <num> <txt>  - insert a line <num> of text <txt>\n\r"
	    				".lr <num> <txt>  - it replaces line <num> with <txt>\n\r"
            				"@                - end string          \n\r", 0 );
            return;
        }
	...
	if (ch->desc->showstr_head)
        {
        	write_to_buffer( ch->desc, "[!!!] The following messages appeared while you were editing:\n\r", 0 );
        	show_string(ch->desc,"");
        }
