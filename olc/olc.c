...
COMMAND(do_clear)
	...
	if (ch->desc->editor != ED_NONE)
		send_to_char("Leaving the editor.\n\r", ch);
	ch->desc->pEdit = NULL;
...
void    do_oedit (CHAR_DATA * ch, char *argument)
{
	...
	if (is_number (arg1))
	{
		...
		{
			send_to_char ("Insufficient security to edit object.\n\r", ch);
			return;
		}
...
void    do_resets (CHAR_DATA * ch, char *argument)
{
	...
	else // arg1 no es un numero
	{
		if ( !str_cmp(arg1, "anadir") )
		{
			...
			if (found == 0)
			{
				printf_to_char(ch, "%s not found in this area.\n\r",
			...
			printf_to_char(ch, "Anadiendo reset del %s %d...", tvar == 1 ? "mob" : "object", found );
...
extern	void UpdateOLCScreen	(DESCRIPTOR_DATA *);
	...
	||  ch->desc->editor == ED_NONE )
	{
		send_to_char( "Nothing to edit.\n\r", ch );
		return;
	}

	if (!is_number(argument))
	{
		send_to_char( "Which page would you like to edit?\n\r", ch );
		return;
	}
	...
	send_to_char( "Page modified, if you do not see anything change to another page.\n\r", ch );
	return;
}
