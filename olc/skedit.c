...
sh_int *gsn_lookup( char *argument )
{
	...
	if ( fBootDb == TRUE )
	{
		sprintf( buf, "gsn_lookup : gsn %s does nto exist", argument );
		bug( buf, 0 );
	}
...
void    skedit (CHAR_DATA * ch, char *argument)
{
	...
	{
		send_to_char ("SKEdit : Security insufficient to edit skill.\n\r", ch);
		edit_done (ch);
		return;
	}
...
void crear_tabla_hash_skills( void )
{
	...
	for ( sn = 0; sn < MAX_SKILL; sn++ )
	{
		...
		if ( valor < 0 || valor > 25 )
		{
			bug( "Crear_tabla_hash_skills : value %d invalid", valor );
			exit(1);
		}
...
SKEDIT( skedit_name )
{
	...
	{
		send_to_char( "A skill/spell with that name already exists.\n\r", ch );
		return FALSE;
	}
...
SKEDIT( skedit_spell )
{
	...
	if ( IS_NULLSTR(argument) )
	{
		send_to_char( "Syntax : spell [name-spell]\n\r", ch );
...
SKEDIT( skedit_gsn )
{
	...
	if ( IS_NULLSTR(argument) )
	{
		send_to_char( "Syntax : gsn [name-gsn]\n\r", ch );
...
SKEDIT( skedit_new )
{
	...
	if ( IS_NULLSTR(argument) )
	{
		send_to_char( "Sintaxis : new [name-of-new-skill]\n\r", ch );
		return FALSE;
	}
	...
	{
		send_to_char ("Failure in realloc. Brace for the impact.\n\r",ch);
		return FALSE;
	}
