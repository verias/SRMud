...
const struct olc_help_type help_table[] =
{
    ...
    {	"off",		off_flags,	"Mob offensive behavior."	},
    {	"size",		size_table,	"Table of sizes."		},
    {   "wclass",       weapon_class,   "Classes of Arms."		},
    {   "wtype",        weapon_type2,   "Special types of Arms."	},
    {	"portal",	portal_flags,	"Types of Portales."		},
    {	"furniture",	furniture_flags,"Types of Furniture."		},
    {	"liquid",	liq_table,	"Types of liquids."		},
    {	"damtype",	attack_table,	"Types of Arms."		},
    ...
    {	"target",	target_table,	"Objective of spells."		},
...
AEDIT( aedit_lowrange )
{
	...
	{
		send_to_char( "Syntax : lowrange [livel-minimum]\n\r", ch );
		return FALSE;
	}
	...
	send_to_char( "Level minimum set.\n\r", ch );
	return TRUE;
}
...
AEDIT( aedit_highrange )
{
	...
	{
		send_to_char( "Argument is lower then low_range.\n\r", ch );
		return FALSE;
	}

	pArea->high_range = high_r;

	send_to_char( "Level max set.\n\r", ch );
	return TRUE;
}
...
bool change_exit( CHAR_DATA *ch, char *argument, int door )
{
	...
	if ( str_cmp( arg, "simple" ) && pToRoom )
	{
		...
		if ( pNExit )
		{
			if ( pNExit->u1.to_room == pRoom )
			{
				...
				printf_to_char( ch, "Exit %d to room %d does not link, exit was not erased.\n\r",
					rev, pToRoom->vnum );
		}
	}
	...
	if (rDeleted)
		printf_to_char( ch, "exit %s to room %d also was deleted.\n\r",
	...
	{
		send_to_char( "REdit: Destination is an area which you cannot edit.\n\r", ch );
		return FALSE;
	}
	...
	&&    pExit->u1.to_room != pRoom )
		printf_to_char( ch, "Warning : the exit in room %d does not link back.\n\r",
			target->vnum );
...
REDIT( redit_mreset )
{
	...
	"There will be a maximum of %d in this area, and %d in this room.\n\r",
...
OEDIT( oedit_show )
{
	...
	{
		send_to_char( "ERROR : Object belongs to an area you cannot edit.\n\r", ch );
		return FALSE;
	}
    }
...
MEDIT( medit_show )
{
	...
	{
		send_to_char( "ERROR : Mob belongs to an area you may not edit.\n\r", ch );
		return FALSE;
	}
    }
...
MEDIT( medit_group )
{
    ...
    if ( argument[0] == '\0' )
    {
    	send_to_char( "Syntax: group [numero]\n\r", ch);
	...
	for ( key = 0; key < MAX_KEY_HASH; ++key )
		for ( obj = obj_index_hash[key]; obj; obj = obj->next )
			if ( obj->reset_num == 0 && (fAll || obj->area == room->area) )
				printf_to_char( ch, "Obj #B%-5.5d#b [%-20.20s] Not this reset.\n\r", obj->vnum, obj->name );
	...
	for ( key = 0; key < MAX_KEY_HASH; ++key )
		for ( mob = mob_index_hash[key]; mob; mob = mob->next )
			if ( mob->reset_num == 0 && (fAll || mob->area == room->area) )
				printf_to_char( ch, "Mob #B%-5.5d#b [%-20.20s] not this reset.\n\r", mob->vnum, mob->player_name );
...
REDIT( redit_copy )
{
	...
	{
		send_to_char( "REdit : room does not exist or is not incopiable.\n\r", ch );
		return FALSE;
	}
...
MEDIT(medit_copy)
{
	...
	if ( IS_NULLSTR(argument) )
	{
		send_to_char( "Syntax : copiar [vnum]\n\r", ch );
		return FALSE;
	}
	...
	{
		send_to_char( "ERROR : Mob belongs to an area you cannot edit.\n\r", ch );
		return FALSE;
	}
...
bool numedit( char * n_fun, CHAR_DATA *ch, char *argument, void * arg, sh_int tipo, long min, long max )
{
	...
	if ( min != -1 && temp < min )
	{
		printf_to_char( ch, "ERROR : Number must be greater then %d.\n\r", min );
		return FALSE;
	}

	if ( max != -1 && temp > max )
	{
		printf_to_char( ch, "ERROR : Number must be less then %d.\n\r", max );
		return FALSE;
	}
...
ED_FUN_DEC( ed_shop )
{
	...
	{
	    send_to_char( "Syntax : shop hours [abrir] [cerrar]\n\r", ch );
	    return FALSE;
	}

	if ( !pMob->pShop )
	{
	    send_to_char( "ERROR : You must create shop first (shop assign).\n\r", ch );
	    return FALSE;
	}
	...
	|| argument[0] == '\0' || !is_number( argument ) )
	{
	    send_to_char( "Syntax : shop profit [%% buys] [%% sells]\n\r", ch );
	    return FALSE;
	}
	...
	if ( !pMob->pShop )
	{
	    send_to_char( "MEdit:  You must create shop first (shop assign).\n\r", ch );
	    return FALSE;
	}
...
ED_FUN_DEC( ed_new_mob )
{
    ...
    if ( !IS_BUILDER( ch, pArea ) )
    {
	send_to_char( "MEdit : that vnum is assigned to an area you cannot edit.\n\r", ch );
	return FALSE;
    }

    if ( get_mob_index( value ) )
    {
	send_to_char( "MEdit : a mob with the vnum already exists\n\r", ch );
	return FALSE;
    }
...
ED_FUN_DEC( ed_objrecval )
{
	...
	{
		default:
		send_to_char( "You cannot recount this object.\n\r", ch );
		return FALSE;
...
bool templookup( char * n_fun, CHAR_DATA *ch, char *argument, void *arg, const void *par, int temp )
{
    ...
    printf_to_char( ch, "Syntax : %s [%s]\n\r"
    			"Type '? %s' for a list of possable arguments.\n\r",
    			n_fun,
    			n_fun,
    			n_fun );
...
ED_FUN_DEC( ed_ed )
{
	...
	*pEd		    =   ed;
	...
	if ( keyword[0] == '\0' )
	{
	    send_to_char( "Syntax:  ed rename [old] [number]\n\r", ch );
	    return FALSE;
	}
	...
	send_to_char( "Extra Description set.\n\r", ch );
	return TRUE;
    }
