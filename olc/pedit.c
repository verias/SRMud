...
void pedit( CHAR_DATA *ch, char *argument)
{
    ...
    {
	send_to_char("MPEdit : Insufficient security to edit code.\n\r",ch);
	edit_done(ch);
	return;
    }
...
void do_pedit(CHAR_DATA *ch, char *argument)
{
    ...
    if ( ch->pcdata->security < MIN_PEDIT_SECURITY )
    {
    	send_to_char( "PEdit : Insufficient security to edit code.\n\r", ch );
    	return;
    }
	...
	{
		send_to_char( "PEdit : Vnum not assigned to any area.\n\r", ch);
		return;
	}
...
MPEDIT (pedit_create)
{
    ...
    {
    	send_to_char("PEdit: Vnum not asigned to any Area.\n\r", ch);
    	return FALSE;
    }

    if ( !IS_BUILDER(ch, pArea) )
    {
    	send_to_char( "PEdit: That vnum is assigned to an area you cannot edit.\n\r", ch );
    	return FALSE;
    }
