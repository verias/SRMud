...
void do_cmdedit(CHAR_DATA *ch, char *argument)
{
    ...
    if (ch->pcdata->security < MIN_CMDEDIT_SECURITY)
    {
    	send_to_char( "CMDEdit : Insufficient security to edit command.\n\r", ch );
    	return;
    }
...
CMDEDIT( cmdedit_list )
{
	...
	{
		send_to_char( "Sintaxis : list [commands/functions] [level min] [level max]\n\r", ch );
		return FALSE;
	}
	...
	if ( !IS_NULLSTR(argument) )
	{
		...
		if ( !is_number( arg2 ) )
		{
			send_to_char( "CMDEdit : argument must be a valid level.\n\r", ch );
			return FALSE;
		}
...
CMDEDIT( cmdedit_name )
{
	...
	if ( IS_NULLSTR(argument) )
	{
		send_to_char( "Syntax : name [number-name]\n\r", ch );
		return FALSE;
	}
	...
	if ( cmd != -1 )
		...
		bugf( "Cmdedit_name : cmd_lookup returned -1 a %s!", argument );
...
CMDEDIT( cmdedit_new )
{
	...
	if ( IS_NULLSTR(argument) )
	{
		send_to_char( "Syntax : new [name-of-new-command]\n\r", ch );
		return FALSE;
	}
	...
	{
		send_to_char ("Error in realloc. Prepare for impact.\n\r",ch);
		return FALSE;
	}
...
CMDEDIT( cmdedit_delete )
{
	...
	if ( IS_NULLSTR(argument) )
	{
		send_to_char( "Syntax : delete [name-of-command-to-remove]\n\r", ch );
		return FALSE;
	}
...
CMDEDIT( cmdedit_funcion )
{
	...
	{
		send_to_char( "CMDEdit : Non-existant function.\n\r", ch );
		return FALSE;
	}
...
CMDEDIT( cmdedit_nivel )
{
	...
	{
		send_to_char( "CMDEdit : Level is out of range.\n\r", ch );
		return FALSE;
	}

	if ( pCmd->level > ch->level )
	{
		send_to_char( "CMDEdit : You cannot do that.\n\r", ch );
		return FALSE;
	}
