...
void	load_groups( void )
{
	...
	if ( !fp )
	{
		bug( "Could not read " GROUP_FILE " to load the groups.", 0 );
		exit(1);
	}
...
void save_groups( void )
{
	...
	if ( !fp )
	{
		bug( "save_groups : fp null when saving races", 0 );
...
void gedit( CHAR_DATA *ch, char *argument)
{
    ...
    {
        send_to_char("SKEdit: Insufficient security to modify group.\n\r",ch);
        edit_done(ch);
        return;
    }
...
void do_gedit(CHAR_DATA *ch, char *argument)
{
    ...
    if (ch->pcdata->security < 5)
    {
    	send_to_char( "GEdit : Insufficient security to edit group.\n\r", ch );
    	return;
    }
