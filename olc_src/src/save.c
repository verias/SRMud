...
extern void known_add	args ((CHAR_DATA *ch, char *name));
...
void fwrite_char( CHAR_DATA *ch, FILE *fp )
{
	...
	fprintf( fp, "Domination	%d\n", ch->domination	);
	fprintf( fp, "Fortitude 	%d\n", ch->fortitude	);
	fprintf( fp, "Obfuscate		%d\n", ch->obfuscate	);
	fprintf( fp, "Potence		%d\n", ch->potence		);
	fprintf( fp, "Presence		%d\n", ch->presence		);
	fprintf( fp, "Protean		%d\n", ch->protean		);
	fprintf( fp, "Quietus		%d\n", ch->quietus		);
    if (ch->trust != 0)
	fprintf( fp, "Tru  %d\n",	ch->trust	);
	fprintf( fp, "Sec  %d\n",	ch->pcdata->security);
...
void fread_char( CHAR_DATA *ch, FILE *fp )
{
	...
	case 'B':
		...
		KEY( "Bsilv",	ch->pcdata->banksilver,	fread_number(fp));
	    KEY( "Bin",		ch->pcdata->bamfin,	fread_string( fp ) );
		KEY( "BirthD", ch->pcdata->birthday, fread_number( fp ) );
		KEY( "BirthM", ch->pcdata->birthmonth, fread_number( fp ) );
		KEY( "BirthY", ch->pcdata->birthyear, fread_number( fp ) );
	...
	case 'C':
		KEY( "Celerity",	ch->celerity,		fread_number( fp ) );
            if ( !str_cmp( word, "Chink" ) )
            {
              ...
              if (gran->do_fun == NULL)
              {
                sprintf(buf,"Chink: Command %s not found in pfile for %s",
                        gran->name,ch->name);
                log_string(buf);
              }  
		...
		KEY( "Cit",		ch->city,		org_lookup(fread_string(fp)));	    
		KEY( "Class",	ch->class,		fread_number( fp ) );
	    KEY( "Cla",		ch->class,		fread_number( fp ) );
	    KEY( "Clan",	ch->clan,	org_lookup(fread_string(fp)));
	...
	case 'D':
		...
		KEY( "Disc",		ch->disc,			fread_number( fp ) );
	...
	case 'G':
		...
		KEY( "Guild",	ch->guild,		org_lookup(fread_string(fp)));
	...
	case 'K':
		    if ( !str_cmp( word, "Known" ) )
            {
              ...
              known->name = str_dup(fread_word(fp));
              known->next = NULL;
	...
	case 'O':
		...
		KEY( "Org",		ch->pcdata->organization, org_lookup(fread_string(fp)));
...
void fread_pet( CHAR_DATA *ch, FILE *fp )
{
    ...
    for ( ; ; )
    {
		...
		paf->where	= fread_number(fp);
    	 ...
    	 case 'C':
             KEY( "Clan",       pet->clan,       org_lookup(fread_string(fp)));
