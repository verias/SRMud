...
//    BUFFER *buffer;
	...
	if (strlen(ch->pnote->text)+strlen(argument) >= MSL)
	{
	...
	send_to_one(ch,"You post a %s regarding %s to %s.",buf,
		ch->pnote->subject, ch->pnote->to_list);
	

	if( !str_cmp(ch->pnote->to_list, "all"))
	{
		send_to_except(ch,"%s has posted a %s regarding %s.",ch->name, buf,
			ch->pnote->subject);
	}
	else
	{
		for(recipient = char_list; recipient; recipient = recipient->next )
		{
			if(!str_cmp(recipient->name,ch->pnote->to_list)
				&& recipient->desc != NULL)
			{
			send_to_one(recipient, "%s has send you a new %s regarding %s.",
				ch->name, buf, ch->pnote->subject);
			}
		}
	}
