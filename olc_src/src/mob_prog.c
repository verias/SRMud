	...
	const void *arg1, const void *arg2, CHAR_DATA *rch )
{
	...
	case CHK_CLAN:
	    return( lval_char != NULL && lval_char->clan == org_lookup( buf ) );
	case CHK_RACE:
	...
	for ( prg = obj->pIndexData->oprogs; prg != NULL; prg = prg->next )
	{
	      ...
	      && strstr( argument, prg->trig_phrase ) != NULL )
	    {
                for(pause = first_pause; pause != NULL; pause = pause->next)
                    if(pause->player == ch && pause->vnum == prg->vnum)
                        return;
		program_flow( prg->vnum, prg->code, NULL, obj, NULL, ch, arg1, arg2, NULL );
		break;
	    }
	}
    }
	...
	for ( prg = room->rprogs; prg != NULL; prg = prg->next )
	{
	      ...
	      && strstr( argument, prg->trig_phrase ) != NULL )
	    {
                for(pause = first_pause; pause != NULL; pause = pause->next)
                    if(pause->player == ch && pause->vnum == prg->vnum)
                        return;
		program_flow( prg->vnum, prg->code, NULL, NULL, room, ch, arg1, arg2, NULL );
		break;
	    }
	}
    }
...
void p_bribe_trigger( CHAR_DATA *mob, CHAR_DATA *ch, int amount )
{
	  ...
	  &&   amount >= atoi( prg->trig_phrase ) )
	{
            for(pause = first_pause; pause != NULL; pause = pause->next)
                if(pause->player == ch && pause->vnum == prg->vnum)
                    return;
	    program_flow( prg->vnum, prg->code, mob, NULL, NULL, ch, NULL, NULL, NULL );
	    break;
	}
    }
return;
		...
		for ( prg = obj->pIndexData->oprogs; prg; prg = prg->next )
		{
		      ...
		      && dir == atoi( prg->trig_phrase ) )
		    {
                        for(pause = first_pause; pause != NULL; pause = pause->next)
                            if(pause->player == ch && pause->vnum == prg->vnum)
                                return TRUE;
			program_flow( prg->vnum, prg->code, NULL, obj, NULL, ch, NULL, NULL, NULL );
			return TRUE;
		    }
		}
	    }
	...
	if ( HAS_TRIGGER_ROOM( room, TRIG_EXALL ) )
	{
		    ...
		    && dir == atoi( prg->trig_phrase ) )
		{
                        for(pause = first_pause; pause != NULL; pause = pause->next)
                            if(pause->player == ch && pause->vnum == prg->vnum)
                                return TRUE;
		    program_flow( prg->vnum, prg->code, NULL, NULL, room, ch, NULL, NULL, NULL );
		    return TRUE;
		}
	    }
	}
		...
		if ( is_number( p ) )
		{
		    if ( dropped->pIndexData->vnum == atoi(p) )
		    {
                        for(pause = first_pause; pause != NULL; pause = pause->next)
                            if(pause->player == ch && pause->vnum == prg->vnum)
                                return;
			program_flow(prg->vnum, prg->code, mob, NULL, NULL, ch, (void *) dropped, NULL, NULL);
			return;
		    }
		}
		/*
	...
	for ( prg = obj->pIndexData->oprogs; prg; prg = prg->next )
	    if ( prg->trig_type == type )
	    {
                for(pause = first_pause; pause != NULL; pause = pause->next)
                    if(pause->obj == obj && pause->vnum == prg->vnum)
                        return;
		program_flow( prg->vnum, prg->code, NULL, obj, NULL, ch, (void *) obj, NULL, NULL );
		return;
	    }
    }
    else if ( room )
		...
		if ( is_number( p ) )
		{
		    if ( dropped->pIndexData->vnum == atoi(p) )
		    {
                         for(pause = first_pause; pause != NULL; pause = pause->next)
                            if(pause->room == room && pause->vnum == prg->vnum)
                                return;
			program_flow( prg->vnum, prg->code, NULL, NULL, room, ch, (void *) dropped, NULL, NULL );
			return;
		    }
		}
		/*
